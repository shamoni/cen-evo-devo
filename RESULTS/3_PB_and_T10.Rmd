...
---
```{r setup,echo=FALSE}
knitr::opts_knit$set(root.dir = '../EXPERIMENTS/7_PB_and_T10')
...
seqdata <- phyclust::read.fasta("q_CEN2_All.aln2")
...
K6ident <-Perident(findbest6_Kmedoids,seqdata$org)
div.df <-cbind(findbest6_Kmedoids$n.class,K6hap,K6ident)
div.df <- as.data.frame(div.df)
colnames(div.df) <-c("No. of seqs","No. of unique seqs","% identity")
...
The F84 matrix tree of all repeats rotated by 90 degrees.
```{r F84_NJtree, echo=FALSE}
load("./RData/findbest6_Kmedoids.RData")
load("./RData/q_CEN2_All_NJtrees.RData")
library(phyclust)
...
plotnj(nj_trees[[4]],type="u",findbest6_Kmedoids$class.id,main=paste("NJ tree, ",mod[4]," matrix", collapse=""),rotate.tree=90)
add.scale.bar(length=0.1,lcol="black",lwd=2)
```
...
edgelabels(tex=round(ret.tree$edge.length,2),bg=rgb(.8,.8,1,.8),cex=0.5)
