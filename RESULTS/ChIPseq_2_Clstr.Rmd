...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
K25.032015 <- read.csv("./2015-03-NChIP-K25/2015-03-NChIP-K25_results.csv", header=T)
K25.082015 <- read.csv("./2015-08-NChIP-K25/2015-08-NChIP-K25_results.csv", header=T)
K25.results <- rbind(K25.032015,K25.082015)
K25.out <-clstr.distribution(K25.results,"ChIP_metadata.csv","Unique")
K25.out.plot <-plot.clstr.distribution(K25.out)
...
K30.out <-clstr.distribution(K30.results,"ChIP_metadata.csv","Unique")
...
nf <-layout(mat,widths=c(1,0.4,1,0.4),heights=1)
custom <-c("#000000","#FF99FFFF","#00E5FFFF","#00FF4DFF","#FFFF00FF","#004CFFFF")
palette(custom)
...
custom <-c(gray(1:10/10)[c(1,4,4)],heat.colors(10)[3],terrain.colors(10)[1],gray(1:10/10)[c(4,4)])
palette(custom)
plot(TAIR10tree, type="u",no.margin=TRUE, show.tip.label=FALSE, edge.color=Cluster)
plot.new()
legend("left",legend=c("Cluster3","Cluster4","Others"), fill = custom[c(4,5,6)], bty="n",cex=1.2,x.intersp=0.2)
```
