...
clstr_grid <- function(df,parse_fun,feature){
    ...
    return(c(list(out.grid),list(dec)))
}

plot_grid <- function(grid,ylab,feature){
    i<-ifelse(feature=="Cluster",1,2)
    col1 <- c("0"="#FFFFFF","1"="#FF4900FF","2"="#00A600FF","3"="#A0D600FF","4"="#FFDB00FF","5"="#00E5FFFF","6"="#0080FFFF","7"="black")
    ...
    p <- p + scale_x_continuous(expand=c(0,0)) + scale_y_continuous(expand=c(0,0),breaks=1:length(ylab),labels=rev(ylab))
    p <- p + theme(axis.ticks=element_blank(), axis.title=element_text(size=14))
    p <- p + labs(x="CEN180 repeats",y="")
    p <- p + theme(axis.text.x=element_blank(), axis.text.y=element_text(size=12,color="black"))
